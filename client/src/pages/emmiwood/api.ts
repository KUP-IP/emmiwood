...
export const emmiwoodApi = {
  ...
  requestCode: (email: string) => request<{ ok: true; previewCode?: string }>('/admin/auth/request-code', { method: 'POST', body: JSON.stringify({ email }) }),
  verifyCode: (email: string, code: string) => request<{ admin: AdminUser }>('/admin/auth/verify', { method: 'POST', body: JSON.stringify({ email, code }) }),
